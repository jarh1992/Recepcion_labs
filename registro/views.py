...
def create_obj(obj_type: str, *req: object) -> object:
    ...
    if obj_type == "student":
        ...
        obj = Pc.objects.create(name=req[0].get("name", ""))
    elif obj_type == "program":
        obj = Program.objects.create(
            name=req[0].get("name", None), cod=req[0].get("cod", None)
        )
    elif obj_type == "loan":
        obj = Loan.objects.create(student=req[1], pc=req[2])
    return obj
                ...
                if loan.departure_time is None:
                    ...
                    response_data = {
                        ...
                        "data": serializers.serialize(
                            "json", (student, student.program)
                        ),
                    }
            except Student.DoesNotExist:
                progs = list(Program.objects.values("name", "cod"))
                response_data = {"response": 1, "programs": progs}
...
def pc_view(request):
    ...
    context = {"list": pc_list, "form_addpc": form_addpc}
